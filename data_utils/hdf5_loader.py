...
class Uniform1024PC(Dataset):
    def __init__(self, root_dir, subdirs, tr_sample_size=4000,
                 te_sample_size=4000, split='train', scale=1.,
        ...
        self.cate_idx_lst = []
        self.all_points = []
        assert os.path.isfile(self.root_dir)

        file = h5py.File(self.root_dir,'r+')
        for cate_idx, subdir in enumerate(self.subdirs):
            for i,data in enumerate(file[subdir][self.split][...]):
                self.all_points.append([data]) 
                self.cate_idx_lst.append(cate_idx)
                self.all_cate_mids.append((subdir, i))
        self.train_sample_num = len(self.all_points)
        ...
        # Normalization
        self.all_points = np.concatenate(self.all_points)  # (N, 1024, 3)
        ...
        self.test_points = self.all_points[:, 5000:]
...
class ShapeNet1024PointClouds(Uniform1024PC):
    def __init__(self, root_dir="data/ShapeNetCore_v2_2048.hdf5",
                 categories=['airplane'], tr_sample_size=1024, te_sample_size=1024,
        ...
        super(ShapeNet1024PointClouds, self).__init__(
...
def get_datasets(args):
    if args.dataset_type == 'shapenet1024':
        tr_dataset = ShapeNet1024PointClouds(
        ...
        te_dataset = ShapeNet1024PointClouds(
...
def get_data_loaders(args):
    ...
    x_train_var = np.var(tr_dataset.train_points.data)
